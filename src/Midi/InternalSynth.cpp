		...
		m_midiSynth(NULL)
{
	...
	if (smallIcon)
	{
		props.AddData("be:mini_icon", 'MICN', smallIcon->Bits(),
...
CInternalSynth::ControlChange(
	...
	bigtime_t time)
{
	m_midiSynth->ControlChange(channel + 1, controlNumber, controlValue,
							   time / 1000);
}
...
void 
CInternalSynth::ChannelPressure(
	uchar channel,
	uchar pressure,
	bigtime_t time)
{
